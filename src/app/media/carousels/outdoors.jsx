"use client";
import { useCategories } from "@/app/lib/firebase/outdoors/read";
  ...
  if (data?.length){
      ...
      data-lenis-prevent>
          {data?.map((category, index) =>{
                  return (                    
                      <SwiperSlide key={index}><Link href={category?.iconURL} data-fancybox="outdoors">
                          <div className="media-card"><ExportedImage src={category?.iconURL} width={800} height={560} loading="lazy" alt={category?.name}/></div>
